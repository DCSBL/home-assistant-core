...
PASSKEY_AUTH_MODULES: Registry[str, type[PasskeyAuthModule]] = Registry()

PASSKEY_AUTH_MODULE_SCHEMA = vol.Schema(
    {
        vol.Optional(CONF_TYPE): object,
        vol.Optional(CONF_NAME): object,
        # Specify ID if you have two mfa auth module for same type.
        vol.Optional(CONF_ID): object,
    },
...
class SetupFlow(data_entry_flow.FlowHandler):
    ...
    def __init__(
        self, auth_module: PasskeyAuthModule, setup_schema: vol.Schema, user_id: str
    ) -> None:
...
async def auth_passkey_module_from_config(
    hass: HomeAssistant, config: dict[str, Any]
) -> PasskeyAuthModule:
    """Initialize an auth module from a config."""
    module_name: str = config[CONF_TYPE]
    module = await _load_passkey_module(hass, module_name)
    ...
    return PASSKEY_AUTH_MODULES[module_name](hass, config)


async def _load_passkey_module(
    hass: HomeAssistant, module_name: str
) -> types.ModuleType:
    """Load an mfa auth module."""
    module_path = f"homeassistant.auth.passkey_modules.{module_name}"

    try:
        module = importlib.import_module(module_path)
    except ImportError as err:
        _LOGGER.error("Unable to load passkey module %s: %s", module_name, err)
        raise HomeAssistantError(
            f"Unable to load passkey module {module_name}: {err}"
        ) from err
